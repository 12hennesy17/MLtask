...
| 2  | Endpoint `/predict` использует **реальную модель**, а не заглушку        |        ✅         | `models/`, реальная модель в папке, используемая в CLI и API                                                                       |  
| 3  | Есть EDA и хотя бы один эксперимент с метриками                          |        ✅         | `notebooks/01_eda.ipynb`, `01_eda_analysis_report.md`, `03_Modeling_and_Evaluation.ipynb`, `03_modeling_evaluation_report.md`     |
| 4  | Есть baseline и улучшенная модель, есть **сравнение по метрикам**        |        ✅         | `03_Modeling_and_Evaluation.ipynb`, `03_modeling_evaluation_report.md`                                              |
| 5  | Код не свален в один ноутбук: есть внятная структура в `src/`            |        ✅         | Например: `src/api/`, `src/cli/`, `src/config/`     |
| 6  | Есть Dockerfile **или** понятный сценарий развёртывания без Docker       |        ✅         | `README.md`, раздел  Как зпустить                   |
